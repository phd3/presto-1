        ...
        implements HivePageSourceFactory
{
    ...
    public Optional<ReaderPageSourceWithProjections> createPageSource(
            ...
            long fileSize,
            Properties schema,
            List<HiveColumnHandle> partialColumns,
            ...
            DateTimeZone hiveStorageTimeZone)
    {
        ...
        Optional<ReaderProjections> projectedReaderColumns = projectBaseColumns(partialColumns);

        List<HiveColumnHandle> columns = projectedReaderColumns
                .map(projection -> projection.getReaderColumns())
                .orElse(partialColumns);
        ...
        try {
            ...
            ConnectorPageSource rcFilePageSource = new RcFilePageSource(rcFileReader, columns);
            return Optional.of(new ReaderPageSourceWithProjections(rcFilePageSource, projectedReaderColumns));
        }
        catch (Throwable e) {
            try {
                inputStream.close();
