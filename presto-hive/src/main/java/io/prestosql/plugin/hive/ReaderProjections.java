...
/**
 * Stores a mapping of columns required by {@link HivePageSource} to columns supplied by format-specific page sources or record cursors.
 */
public class ReaderProjections
{
    ...
    private ReaderProjections(List<HiveColumnHandle> readerColumns, List<Integer> readerBlockIndices)
    {
        ...
        readerBlockIndices.stream().allMatch(value -> value >= 0 && value < readerColumns.size());
    ...
    /**
     * Creates unique base columns for input projected columns and prepares a mapping between the two.
     */
    public static Optional<ReaderProjections> projectBaseColumns(List<HiveColumnHandle> columns)
    {
        requireNonNull(columns, "columns is null");

        if (columns.stream().allMatch(column -> column.isBaseColumn())) {
        ...
        for (int index = 0; index < columns.size(); index++) {
            HiveColumnHandle column = columns.get(index);
    ...
    public static Optional<ReaderProjections> projectSufficientColumns(List<HiveColumnHandle> columns)
    {
        ...
        for (HiveColumnHandle column : columns) {
            List<Integer> indices = column.getHiveColumnProjectionInfo()
                    .map(info -> info.getDereferenceIndices())
        ...
        for (int i = 0; i < columns.size(); i++) {
            DereferenceChain column = dereferenceChains.inverse().get(columns.get(i));
            ...
            for (int length = 0; length < orderedPrefixes.size(); length++) {
                DereferenceChain prefix = orderedPrefixes.get(length);
