        ...
        extends BaseRuleTest
{
    ...
    public void testProjectionPushdown()
    {
        ...
        tester().getQueryRunner().execute("CREATE TABLE " +
                HIVE_CATALOG_NAME + "." + SCHEMA_NAME + "." + TABLE_NAME + "\n" +
                "(struct_of_int)" +
                " AS " +
        ...
        tester().assertThat(pushProjectionIntoTableScan)
                .on(p -> p.project(
                    ...
                    p.tableScan(table, ImmutableList.of(p.symbol("struct_of_int", baseType)), ImmutableMap.of(p.symbol("struct_of_int", baseType), fullColumn))))
                .withSession(testSessionBuilder().setCatalog(HIVE_CATALOG_NAME).setSchema(SCHEMA_NAME).build())
                .doesNotFire();
        ...
        tester().assertThat(pushProjectionIntoTableScan)
                ...
                .matches(project(
                    ...
                    tableScanWithProvidedTableAndColumns(table.getConnectorHandle(), TupleDomain.all(), ImmutableMap.of("struct_of_int#a", partialColumn))));
    }
