...
public class TestReaderProjections
{
    ...
    public void testBaseColumnsProjection()
    {
        List<HiveColumnHandle> columns = ImmutableList.of(
                createPartialColumnHandle(TEST_FULL_COLUMNS.get("col_struct_of_primitives"), ImmutableList.of(0)),
                createPartialColumnHandle(TEST_FULL_COLUMNS.get("col_struct_of_primitives"), ImmutableList.of(1)),
                createPartialColumnHandle(TEST_FULL_COLUMNS.get("col_bigint"), ImmutableList.of()),
                createPartialColumnHandle(TEST_FULL_COLUMNS.get("col_struct_of_non_primitives"), ImmutableList.of(0, 1)),
                createPartialColumnHandle(TEST_FULL_COLUMNS.get("col_struct_of_non_primitives"), ImmutableList.of(0)));

        Optional<ReaderProjections> mapping = projectBaseColumns(columns);
        assertTrue(mapping.isPresent(), "Full columns should be projected from partial columns");
