...
//            .add(new TestColumn("p_null_binary", javaByteArrayObjectInspector, HIVE_DEFAULT_DYNAMIC_PARTITION, null, true))
    ...
    public static final class TestColumn
    {
        ...
        public static TestColumn getPartialTestColumn(TestColumn parent, List<String> dereferenceNames, Object writeValue, Object expectedValue)
        {
            String baseName = parent.getBaseName();
            ImmutableList.Builder<String> partialColumnFieldNames = ImmutableList.builder();
            partialColumnFieldNames.addAll(parent.getDereferenceNames());

            ImmutableList.Builder<Integer> partialColumnFieldIndices = ImmutableList.builder();
            partialColumnFieldIndices.addAll(parent.getDereferenceIndices());

            ObjectInspector currentObjectInspector = parent.getObjectInspector();

            for (int i = 0; i < dereferenceNames.size(); i++) {
                StructObjectInspector structInspector = (StructObjectInspector) currentObjectInspector;

                StructField field = structInspector.getStructFieldRef(dereferenceNames.get(i));
                currentObjectInspector = field.getFieldObjectInspector();

                partialColumnFieldNames.add(field.getFieldName());
                partialColumnFieldIndices.add(field.getFieldID());
            }

            return new TestColumn(
                    baseName,
                    parent.getBaseObjectInspector(),
                    partialColumnFieldNames.build(),
                    partialColumnFieldIndices.build(),
                    currentObjectInspector,
                    writeValue,
                    expectedValue,
                    parent.isPartitionKey());
        }
    }
}
