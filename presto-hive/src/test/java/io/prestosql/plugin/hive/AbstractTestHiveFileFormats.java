...
//            .add(new TestColumn("p_null_binary", javaByteArrayObjectInspector, HIVE_DEFAULT_DYNAMIC_PARTITION, null, true))
    ...
    protected List<HiveColumnHandle> getColumnHandles(List<TestColumn> testColumns)
    {
        ...
        for (int i = 0; i < testColumns.size(); i++) {
            ...
            columns.add(createTopLevelHiveColumnHandle(testColumn.getName(), columnIndex, hiveType, hiveType.getType(TYPE_MANAGER), testColumn.isPartitionKey() ? PARTITION_KEY : REGULAR, Optional.empty()));
        }
        return columns;
    }
